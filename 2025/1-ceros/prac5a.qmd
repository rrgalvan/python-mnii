...
2. Programar una función `newton(f, df, x0, tol=1e-15, maxiter=100)` aproxime el cero de `f` (con derivada `df`) mediante el método de Newton partiendo de `x0`. La función realizará un máximo de `maxiter` iteraciones, deteniéndose si la diferencia entre dos iteraciones consecutivas es menor que la tolerancia `tol`, y devuelva una tupla `(x,k)` dodne `x` es la última aproximación y `k` es el número de iteraciones realizadas.
...
f(2), f(5)
```
El siguiente código queda más bonito y la última línea daría un error si no se verifican las condiciones de Bolzano (puedes comprobarlo)
```{python}
a, b = 2, 5
...
fig, ax = plt.subplots(1, 3, figsize=(10,3))   # Tamaño 6 x 3

a, b = 2, 5
x = np.linspace(a, b, num=300)

ax[0].set_title("$f(x)$", c="blue")
ax[0].plot(x, f(x), c="blue")

ax[1].set_title("$f'(x)$", c="red")
ax[1].plot(x, df(x), c="red")

ax[2].set_title("$f''(x)$", c="darkgreen")
ax[2].plot(x, df2(x), c="darkgreen")
...
```{python}
aprox, niter = newton(f, df, x0=3.5)
